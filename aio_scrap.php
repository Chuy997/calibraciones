<?php
// /var/www/html/calibraciones/aio_scrap.php
declare(strict_types=1);

require_once __DIR__ . '/config.php';
require_auth(['admin','ingenieria','aio']);
require_once __DIR__ . '/aio_scrap_mail.php';
  ...
  if (!isset($_POST['csrf']) || !csrf_validate($_POST['csrf'])) {
      ...
      if (empty($ids)) {
        ...
        try {
          ...
          foreach ($ids as $did) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM aio_items WHERE ID=:id AND Status='Scrap'");
            $stmt->execute([':id' => $did]);
            $r = $stmt->fetch();
            if (!$r) continue;

            $pdo->prepare("UPDATE aio_items SET Status='Destruido', UpdatedAt=NOW() WHERE ID=:id")
              ->execute([':id' => $did]);

            $pdo->prepare("INSERT INTO aio_history
                            (AioID,Action,Description,Brand,Model,SerialNumber,
    ...
    if ($action === 'send_email') {
      ...
      if (empty($pending)) {
        ...
        if (aio_send_scrap_email($pending)) {
...
?>
  ...
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
    ...
    <div class="d-flex gap-2 flex-wrap">
      <!-- Botón Enviar correo -->
      <form method="POST" action="aio_scrap.php" id="emailForm" class="d-inline">
      ...
      <a class="btn btn-outline-secondary btn-sm" href="aio_admin.php">
  ...
  <?php else: ?>
    ...
    <div id="bulkBar" class="card p-2 mb-3 border-warning d-none">
      <form method="POST" action="aio_scrap.php" id="destroyForm">
        <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
                  ...
                  <td>
                    <div class="btn-group">
                      <a class="btn btn-primary btn-sm"
                        href="aio_update.php?id=<?= urlencode((string)$r['ID']) ?>" title="Editar">
                        <i class="fa fa-pen-to-square"></i>
                      </a>
                      <a class="btn btn-info btn-sm"
                        href="aio_history.php?id=<?= urlencode((string)$r['ID']) ?>" title="Historial">
...
<div class="row justify-content-center">
  <div class="col-12 col-lg-8 col-xl-7">
    <div class="d-flex align-items-center gap-3 my-3">
      <a href="aio_scrap.php" class="btn btn-outline-secondary btn-sm">
    ...
    <?php if ($item['Status'] === 'Scrap'): ?>
      ...
      <a href="aio_scrap.php" class="btn btn-outline-secondary">Ver listado Scrap</a>
    <?php else: ?>
      <form method="POST" action="aio_scrap.php" class="needs-validation" novalidate>
        <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
        ...
        <div class="d-flex gap-2">
          <a href="aio_scrap.php" class="btn btn-outline-secondary">Cancelar</a>
