<?php
// /var/www/html/calibraciones/aio_report_print.php
...
// 1. Audit Header
$st = $pdo->prepare("SELECT * FROM aio_audits WHERE AuditID = ?");
$st->execute([$id]);
$audit = $st->fetch();

if (!$audit) die("Auditoría no encontrada.");

// 2. Audit Items + AIO Details
$sqlItems = "
    SELECT 
        ai.*, 
        gi.Description, 
    ...
    FROM aio_audit_items ai
    JOIN aio_items gi ON ai.AioID = gi.ID
    WHERE ai.AuditID = ?
    ORDER BY ai.AioID ASC
";
...
<body>
    ...
    <div class="header">
        <div>
            <h1>Reporte de Auditoría: Material AIO</h1>
            <div class="meta">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><strong><?= h($item['AioID']) ?></strong></td>
                <td>
