<?php
// /var/www/html/calibraciones/golden_package_create.php
    ...
    if (!isset($_POST['csrf']) || !csrf_validate($_POST['csrf'])) {
        ...
        if (!$errors) {
            try {
                ...
                // Generate PackageID
                $pkgId = 'PKG-' . date('Ymd') . '-' . strtoupper(bin2hex(random_bytes(3)));
                
                // Insert Package
                $stmt = $pdo->prepare("
                    INSERT INTO golden_packages (PackageID, Name, Description, Location, CreatedBy)
                ...
                $upd = $pdo->prepare("UPDATE golden_items SET PackageID = ?, Location = ? WHERE ID IN ($placeholders)");
                $upd->execute(array_merge([$pkgId, $loc], $selectedItems));

                // Log History
                $hist = $pdo->prepare("
                    INSERT INTO golden_history (GoldenID, Action, Description, Status, Comments, CreatedAt)
                    SELECT ID, 'package_add', Description, Status, CONCAT('Added to Package ', :pkgId), NOW()
                    FROM golden_items WHERE PackageID = :pkgId2
                ");
...
<div class="row justify-content-center">
    <div class="col-12 col-xl-10">
        <h1 class="h4 my-3">Crear Nuevo Paquete</h1>
        ...
        <form method="POST" class="needs-validation" novalidate onsubmit="return validateForm()">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" required placeholder="Ej. Kit de Mantenimiento A">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Ubicación Inicial</label>
                            <input type="text" name="location" class="form-control" placeholder="Ej. Almacén Central">
                        </div>
                        <div class="col-12">
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="golden_packages.php" class="btn btn-outline-secondary">Cancelar</a>
