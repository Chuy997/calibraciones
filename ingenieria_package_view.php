<?php
// /var/www/html/calibraciones/ingenieria_package_view.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_location') {
        ...
        if ($newLoc) {
            ...
            // Update Package
            $stmt = $pdo->prepare("UPDATE ingenieria_packages SET Location = :loc WHERE PackageID = :id");
            $stmt->execute([':loc' => $newLoc, ':id' => $id]);
            
            // Update Items
            $stmt2 = $pdo->prepare("UPDATE ingenieria_items SET Location = :loc WHERE PackageID = :id");
    ...
    elseif ($action === 'disband') {
        ...
        $pdo->beginTransaction();
        // Archive Package
        $stmt = $pdo->prepare("UPDATE ingenieria_packages SET Status = 'Archived' WHERE PackageID = :id");
        $stmt->execute([':id' => $id]);
        
        // Release items
        $stmt2 = $pdo->prepare("UPDATE ingenieria_items SET PackageID = NULL WHERE PackageID = :id");
        $stmt2->execute([':id' => $id]);
    ...
    header("Location: ingenieria_package_view.php?id=$id");
    exit;
}
...
// Fetch Items
$stmtItems = $pdo->prepare("SELECT * FROM ingenieria_items WHERE PackageID = :id ORDER BY ID ASC");
...
<div class="row mb-4">
    <div class="col-12 d-flex justify-content-between align-items-center">
        ...
        <div class="btn-group">
            <a href="ingenieria_packages.php" class="btn btn-outline-secondary"><i class="fa fa-arrow-left"></i> Volver</a>
            <a href="ingenieria_package_label.php?id=<?= h($pkg['PackageID']) ?>" target="_blank" class="btn btn-dark">
            ...
            <div class="table-responsive">
                <table class="table table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><a href="ingenieria_history.php?id=<?= h($item['ID']) ?>"><?= h($item['ID']) ?></a></td>
                            <td>
