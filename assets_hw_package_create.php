<?php
// /var/www/html/calibraciones/assets_hw_package_create.php
    ...
    if (!isset($_POST['csrf']) || !csrf_validate($_POST['csrf'])) {
        ...
        if (!$errors) {
            try {
                ...
                // Generate PackageID (APKG for Assets Package)
                $pkgId = 'APKG-' . date('Ymd') . '-' . strtoupper(bin2hex(random_bytes(3)));
                
                // Insert Package
                $stmt = $pdo->prepare("
                    INSERT INTO assets_hw_packages (PackageID, Name, Description, Location, CreatedBy)
                ...
                $upd = $pdo->prepare("UPDATE assets_hw_items SET PackageID = ?, Location = ? WHERE ID IN ($placeholders)");
                $upd->execute(array_merge([$pkgId, $loc], $selectedItems));

                // Log History
                $hist = $pdo->prepare("
                    INSERT INTO assets_hw_history (AssetsHWID, Action, Description, Status, Comments, CreatedAt)
                    SELECT ID, 'package_add', Description, Status, CONCAT('Added to Package ', :pkgId), NOW()
                    FROM assets_hw_items WHERE PackageID = :pkgId2
                ");
                $hist->execute([':pkgId' => $pkgId, ':pkgId2' => $pkgId]);

                $pdo->commit();
                header("Location: assets_hw_packages.php");
                exit;
...
$items = $pdo->query("
    SELECT ID, Description, Brand, Model, SerialNumber, Location, Pedimento
...
<div class="row justify-content-center">
    <div class="col-12 col-xl-10">
        <h1 class="h4 my-3">Crear Paquete de Activos</h1>
        ...
        <form method="POST" class="needs-validation" novalidate onsubmit="return validateForm()">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" required placeholder="Ej. Lote de PCs Ingenieros">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Ubicación Inicial</label>
                            <input type="text" name="location" class="form-control" placeholder="Ej. Rack Server 1">
                        </div>
                        <div class="col-12">
            ...
            <div class="card mb-4">
                <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                    <span>Selección de Activos</span>
                ...
                <div class="card-body p-0" style="max-height: 500px; overflow-y: auto;">
                    <table class="table table-hover table-striped mb-0" id="itemsTable">
                        ...
                        <tbody>
                            <?php if (!$items): ?>
                                <tr><td colspan="6" class="text-center py-3">No hay activos disponibles.</td></tr>
                            <?php else: foreach ($items as $item): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($item['Pedimento']) ?></td>
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="assets_hw_packages.php" class="btn btn-outline-secondary">Cancelar</a>
