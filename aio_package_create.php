<?php
// /var/www/html/calibraciones/aio_package_create.php
    ...
    if (!isset($_POST['csrf']) || !csrf_validate($_POST['csrf'])) {
        ...
        if (!$errors) {
            try {
                ...
                // Generate PackageID (IPKG for AIO Package)
                $pkgId = 'IPKG-' . date('Ymd') . '-' . strtoupper(bin2hex(random_bytes(3)));
                
                // Insert Package
                $stmt = $pdo->prepare("
                    INSERT INTO aio_packages (PackageID, Name, Description, Location, CreatedBy)
                ...
                $upd = $pdo->prepare("UPDATE aio_items SET PackageID = ?, Location = ? WHERE ID IN ($placeholders)");
                $upd->execute(array_merge([$pkgId, $loc], $selectedItems));

                // Log History
                $hist = $pdo->prepare("
                    INSERT INTO aio_history (AioID, Action, Description, Status, Comments, CreatedAt)
                    SELECT ID, 'package_add', Description, Status, CONCAT('Added to Package ', :pkgId), NOW()
                    FROM aio_items WHERE PackageID = :pkgId2
                ");
    ...
    <div class="col-12 col-xl-10">
        ...
        <form method="POST" class="needs-validation" novalidate onsubmit="return validateForm()">
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="aio_packages.php" class="btn btn-outline-secondary">Cancelar</a>
