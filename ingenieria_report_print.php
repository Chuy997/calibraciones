<?php
// /var/www/html/calibraciones/ingenieria_report_print.php
...
// 1. Audit Header
$st = $pdo->prepare("SELECT * FROM ingenieria_audits WHERE AuditID = ?");
$st->execute([$id]);
$audit = $st->fetch();

if (!$audit) die("Auditoría no encontrada.");

// 2. Audit Items + Ingenieria Details
$sqlItems = "
    SELECT 
        ai.*, 
        gi.Description, 
    ...
    FROM ingenieria_audit_items ai
    JOIN ingenieria_items gi ON ai.IngenieriaID = gi.ID
    WHERE ai.AuditID = ?
    ORDER BY ai.IngenieriaID ASC
";
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><strong><?= h($item['IngenieriaID']) ?></strong></td>
                <td>
